...
def dashboard(request):
    ...
    total_pos_sales = POS.objects.all().aggregate(Sum('total_amount'))['total_amount__sum']
    # sum of total ecommerce sale
    total_ecom_sales = Order.objects.all().aggregate(Sum('total_price'))['total_price__sum']
    # active coupons
    active_coupons = Cupon.objects.filter(is_active=True).count()
    #total revenue 
    total_revenue = Product.objects.aggregate(Sum('total_revenue'))['total_revenue__sum']
    #total profit
    total_profit = Product.objects.aggregate(Sum('total_profit'))['total_profit__sum']
    #total due
    total_due = POS.objects.all().aggregate(Sum('due_amount'))['due_amount__sum']
